...
vector<Plate> DetectRegions::segment(Mat input){
    ...
    Sobel(img_gray, img_sobel, CV_16S, 1, 0, 3, 1, 0, BORDER_DEFAULT);
    ...
    vector< vector< Point> > contours;
            ...
            //image   Source, an 8-bit single-channel image. Non-zero pixels are treated as 1's. 
    ...
    for(int i=0; i< rects.size(); i++){
        ...
        // the minium bounding rectangle for the floodfilled area 
        RotatedRect minRect = minAreaRect(pointsInterest);


        // IF the area may be the plate
        if(verifySizes(minRect)){
            ...
            //imshow("before",grayResult);

            //Each image contains different light conditions, inscreasing their relative differences
            grayResult=histeq(grayResult);
            //imshow("after",grayResult);
            
            if(saveRegions){ 
                stringstream ss(stringstream::in | stringstream::out);
                ss << "./tmp/" << filename << "_" << i << ".jpg";
                imwrite(ss.str(), grayResult);
                imshow(ss.str(), grayResult);
            }
            output.push_back(Plate(grayResult,minRect.boundingRect()));
            

            //rectangle( img_rotated, minRect.boundingRect(), Scalar( 0, 255, 255 ), FILLED, LINE_8 );
            //imshow("aa",img_rotated);
        }
    }       
    if(showSteps) 
